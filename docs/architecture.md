...
Dynamic probes are emitted only by an explicit Tab evaluation of a trusted pack. They use direct argument vectors rather than shell command strings and reject relative/empty PATH components and loader/startup-hook environment variables. The supervisor starts the immutable, co-installed `bashlume-probe` trampoline with `posix_spawnp`; the trampoline closes every inherited descriptor above standard error and applies process, address-space, descriptor, CPU, core, and file-size limits before it `execvp`s the declared target, closing the former descriptor-leak and post-spawn `prlimit` races. Because Linux charges `RLIMIT_NPROC` to the real UID, its finite limit is the fail-closed `/proc` task baseline plus sixteen probe slots rather than an absolute value that would disable probes in an active desktop session. It also installs `no_new_privs` and an inherited seccomp filter that denies `setpgid` and `setsid`. Every target and descendant therefore remains in the fresh probe process group. The supervisor observes an exited leader with `waitid(..., WNOWAIT)`, kills that still-unreusable group, and only then reaps it, so a descendant cannot survive by closing pipes or outliving its leader. Output and wall time remain independently bounded, and timeout publication never waits for pipe EOF. Generation-based cancellation is out-of-band and remains polled until acknowledged. Ordinary typing and ghost evaluation never spawn processes.
